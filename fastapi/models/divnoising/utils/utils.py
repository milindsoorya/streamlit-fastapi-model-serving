    ...
    for i in range(1):
        ...
        if fraction_samples_to_export>0:
            ...
            if not os.path.exists(subdir):
                os.makedirs(subdir)
            imsave(subdir+"samples_for_image_"+str(i).zfill(3)+".png",
                   np.array(samples[:int(num_samples*fraction_samples_to_export)]).astype("float32"))
            
    if (export_mmse):    
        imsave(export_results_path+"/mmse_results.png", np.array(mmse_results).astype("float32"))
    return mmse_results

def predict_and_save_tiff(img,vae,num_samples,device,
                     fraction_samples_to_export,export_mmse,export_results_path,tta):
...
def image_from_array(noisy_input,vae,device):
    '''
    Plot qualitative results on patches.
    Parameters
    ----------
